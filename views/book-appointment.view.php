        ...
        <section class="login">
            <div class="container">
                 <!-- a form that uses the POST method to submit data to the appointment.php -->
                <form method="POST" action="./book-appointment.php" class="form" id="appointment-form">
                    <h3>Book Appointment</h3>
                    
                    <!-- Error Messages -->
                    <?php error_msg("error_booking_appointment", "There was an error booking appointment, Please try again!"); ?>
                    ...
                    <div class="form-services">
                        <?php
                            ...
                            $stmt = $dbconn->prepare($sql);//preparing an SQL statement
                            $stmt->execute();//executing the prepared statement
                            $result = $stmt->get_result();//result obtained
                            ...
                            if(mysqli_num_rows($result)>0){// this function checks if the result stored in the variable $result has one or more rows.
                                while($row = mysqli_fetch_assoc($result)){//if rows exists, this while loop fetches for each row using mysqli_fetch_assoc($result)
                                    ...
                                    <div class="service-checkbox">
                                        <input type="checkbox" name="service[]" value="<?php echo $service_name; ?>" id="service-<?php echo $id; ?>" data-price="<?php echo $amount; ?>" class="service-check">
                                        <label for="service-<?php echo $id; ?>"><?php echo $service_name . ", Ksh " . $amount ; ?></label>
                                    </div>
                    ...
                    <input type="time" name="time" id="appointmentTime">
                    <label for="amount">Amount</label>
                    <input type="number" name="amount" id="amount" value="" readonly id="amount">
...
<script>
    ...
    //selecting an HTML element with the various IDs and assigning them to their variables 
    ...
    // adds an event listener to the Form element for the "submit" event. When the form is submitted, the callback function(e.preventDefault()) is executed.
    appointmentForm.addEventListener("submit", e=>{
        e.preventDefault();

        checkAppointments();//calling the function
    });

    let checkAppointments = ()=> {//definig the function
