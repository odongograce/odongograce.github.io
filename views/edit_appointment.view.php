<section class="login">
            <div class="container">
                 <!-- a form that uses the POST method to submit data to the edit_appointment.php -->
                <form method="POST" action="./edit_appointment.php" class="form" id="appointment-form">
                            ...
                            if(mysqli_num_rows($result)>0){// this function checks if the result stored in the variable $result has one or more rows.
                                while($row = mysqli_fetch_assoc($result)){//if rows exists, this while loop fetches for each row using mysqli_fetch_assoc($result)
                                    ...
                                    <div class="service-checkbox">
                                        <input type="checkbox" name="service[]" value="<?php echo $service_name; ?>" id="service-<?php echo $id; ?>" data-price="<?php echo $amount; ?>" <?php echo (in_array($service_name, $services))? 'checked': ''; ?> class="service-check">
                    ...
                    <input type="text" name="amount" id="amount" value="<?php echo $totalAmount; ?>" readonly>
